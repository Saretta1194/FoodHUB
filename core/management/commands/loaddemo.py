...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):
        # Users
        owner, _ = User.objects.get_or_create(username="owner1", defaults={"email": "owner1@example.com"})
        if not owner.has_usable_password():
            owner.set_password("pass123"); owner.save()
        ...
        if not customer.has_usable_password():
            customer.set_password("pass123"); customer.save()

        staff, _ = User.objects.get_or_create(username="operator", defaults={"email": "operator@example.com", "is_staff": True})
        ...
        rider, _ = User.objects.get_or_create(username="rider1", defaults={"email": "rider1@example.com"})
        if not rider.has_usable_password():
            rider.set_password("pass123"); rider.save()
        ...
        dishes_r1 = [
            ...
            ("Quattro Formaggi", "Mozzarella, gorgonzola, grana, provola", Decimal("9.00"), True),
            ("Marinara", "Tomato, garlic, oregano", Decimal("5.50"), True),
            ("Prosciutto e Funghi", "Cooked ham, mushrooms, mozzarella", Decimal("9.50"), True),
        ]
        dishes_r2 = [
            ("Sushi Mix 12pz", "Nigiri + Maki + Uramaki", Decimal("15.90"), True),
            ("Sashimi Salmone 10pz", "Fresh salmon slices", Decimal("13.50"), True),
            ("Uramaki California 8pz", "Crabstick, avocado, mayo", Decimal("7.90"), True),
        ...
        for name, desc, price, available in dishes_r1:
            Dish.objects.get_or_create(restaurant=r1, name=name, defaults={"description": desc, "price": price, "available": available})
        for name, desc, price, available in dishes_r2:
            Dish.objects.get_or_create(restaurant=r2, name=name, defaults={"description": desc, "price": price, "available": available})
        for name, desc, price, available in dishes_r3:
            Dish.objects.get_or_create(restaurant=r3, name=name, defaults={"description": desc, "price": price, "available": available})
