...
class AssignmentTests(TestCase):
    def setUp(self):
        # Users
        self.operator = User.objects.create_user(username="operator", password="pass123", is_staff=True)
        ...
        OrderItem.objects.create(order=self.order, dish=Dish.objects.create(
            restaurant=self.rest, name="Pasta", price=Decimal("10.00"), available=True
        ), dish_name="Pasta", unit_price=Decimal("10.00"), quantity=1)
    ...
    def test_operator_can_assign_rider_and_event_logged(self):
        ...
        # event logged
        self.assertTrue(DeliveryEvent.objects.filter(delivery=d, event_type=DeliveryEvent.EVENT_ASSIGNED).exists())
