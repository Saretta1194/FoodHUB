...
class RiderStatusFlowTests(TestCase):
    def setUp(self):
        ...
        self.customer = User.objects.create_user(username="cust", password="pass123", email="cust@example.com")
    ...
    def test_forward_only_and_timeline_and_notification(self):
        ...
        url_delivered = reverse("deliveries:rider_mark_delivered", args=[self.delivery.pk])
        ...
        # Timeline has events
        self.assertTrue(DeliveryEvent.objects.filter(delivery=self.delivery, event_type=DeliveryEvent.EVENT_STATUS_CHANGE).exists())
